...
| **FT V3 (this pipeline, 1425 samples)** | **60.0% (±0.9)** | **54.0% (±1.8)** |

**V3 vs base:** HumanEval **+4.5 pts (+8.1% relative)**, HumanEval+ **+5.0 pts (+10.2% relative)**. Error bars don't overlap — the difference is statistically significant.

The V3 jump over V1 came from stacking the pipeline's features end-to-end: embedding-based deduplication (−75 near-duplicates out of 1500), tighter per-category descriptions, and higher-diversity multi-turn generation — all controlled from the UI, no scripting.
